...
Configure transactional email delivery so the application can send confirmation,
password-reset, and other Devise emails reliably in production and catch them
...
using the `letter_opener_web` gem with `delivery_method: :letter_opener_web`,
mounted at `/letter_opener`.

#### Scenario: Developer triggers a confirmation email locally
- **WHEN** a developer signs up a user in the development environment
- **THEN** the confirmation email is viewable at `/letter_opener` instead of being
