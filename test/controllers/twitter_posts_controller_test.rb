...
class TwitterPostsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "new redirects to a generate_twitter_post chat owned by the parent script" do
    get new_script_twitter_post_path(@script)

    assert_redirected_to new_chat_path(purpose: "generate_twitter_post",
  ...
  test "create saves the post and redirects to its show page" do
    assert_difference("TwitterPost.count", 1) do
      post script_twitter_post_path(@script),
           params: { twitter_post: { title: "My thread", hook: "h", body: "b" } }
    end
    assert_redirected_to script_twitter_post_path(@script)
  end

  test "create re-renders new with an invalid post" do
    assert_no_difference("TwitterPost.count") do
      post script_twitter_post_path(@script),
           params: { twitter_post: { title: "" } }
    end
    assert_response :unprocessable_entity
  end

  test "show renders the post" do
    @script.create_twitter_post!(title: "My thread", hook: "h", body: "b")
    get script_twitter_post_path(@script)
    assert_response :success
  end

  test "update changes the post and redirects" do
    @script.create_twitter_post!(title: "Old", hook: "h", body: "b")
    patch script_twitter_post_path(@script),
          params: { twitter_post: { title: "New" } }
    assert_redirected_to script_twitter_post_path(@script)
    assert_equal "New", @script.reload.twitter_post.title
  end

  test "destroy removes the post and redirects to the script" do
    @script.create_twitter_post!(title: "Doomed", hook: "h", body: "b")
    assert_difference("TwitterPost.count", -1) do
      delete script_twitter_post_path(@script)
    end
  ...
  test "cannot reach another user's script" do
    ...
    get new_script_twitter_post_path(other_script)
